...
* 其他情况
  * 线程异常
    * 例如主程序启动了一个线程，随后线程执行过程中出现了RuntimeException，如果未捕获这个异常，那么线程直接死亡，导致程序莫名宕机。处理方法
      * 可以在线程内进行捕获，随后抛出ThreadDeath异常
      * 可以在祝线程实现UncaughtExceptionHandler接口，随后在线程start前启动
- - -
